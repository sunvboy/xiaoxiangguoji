...
<tr @if($v->type == 0) style="opacity: 0.5" @endif class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 odd" id="post-<?php echo $v->id; ?>">
    ...
    <td class="p-2">
        <a href="{{!empty($v->catalogue_id == 2) ? route('deals.website.edit',['id'=>$v->id]) : route('deals.edit',['id'=>$v->id]) }}" class="font-semibold underline">{{$v->title}}</a>
    </td>
    @endif
    ...
    @if($v->catalogue_id == 1 || $v->catalogue_id == 21199)
