    ...
    <div class="container px-4 md:px-0">
        ...
        <div class="grid grid-111 grid-cols-1 md:grid-cols-3 gap-8">
            <div class="space-y-3 col-span-2 bg-[#fff] p-[16px] rounded-2xl box-item-1">
                <h1 class="font-bold text-[22px] tracking-[0.7px] uppercase hidden">{{$page->title}}</h1>
                ...
                ?>
                <div class="cart_item cart-{{$k}} flex gap-5 items-center border-b pb-3">
                    <div class="w-[390px] flex gap-5 items-center">
                        <a href="{{$slug}}" class="flex w-[62px] border rounded-[7.2px] p-[4px]">
                            <img alt="{{$item['title']}}" src="{{asset($item['image'])}}" class="w-full h-[52px] rounded-lg">
                        </a>
                        <div class="flex flex-col flex-1">
                            <span class="title-3">{{$item['title']}}</span>
                            <span class="title-4">{{$title_version}}</span>
                        </div>
                    </div>
                    <div class="w-[200px] text-center flex flex-col">
                        <span class="text-red-600 font-semibold">{{number_format($item['price'])}}đ</span>
                        @if(!empty($item['price_old']))
                        <span class="line-through">{{number_format($item['price_old'])}}đ</span>
                        @endif
                    </div>
                    <div class="flex items-center w-[130px]" style="width: 130px;">
                        <div class="flex items-center flex-1">
                            <button data-rowid="{{$k}}" class="tp_cart_minus flex justify-center items-center w-[25px] !h-[32px] border-t border-b border-l" style="border-radius: 34px 0px 0px 34px;">
                            ...
                            <input type="number" min="1" data-rowid="{{$k}}" max="{{$stock}}" step="1" value="{{$item['quantity']}}" class="flex-auto w-8 !h-[32px] text-center border focus:outline-none input-appearance-none tp_cardQuantity">
                            <button data-rowid="{{$k}}" class="tp_cart_plus flex justify-center items-center w-[25px] !h-[32px]  border-t border-b border-r" style="border-radius: 0 34px 34px 0;">
                                <svg class="w-6" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="bg-[#fff] rounded-tl-2xl rounded-tr-2xl max-w-[384px] mx-auto box-ma">
                    <div class="p-[16px]">
                        ...
                        <div class="pt-[20px] border-t">
                            ...
                            <a class="ps-btn ps-btn--warning" href="{{route('cart.checkout')}}">Thanh toán</a>
        ...
        <div class="flex flex-col justify-center items-center space-y-5">
            <picture><img src="{{asset('images/empty-cart.png')}}" alt="img"></picture>
            <div class="text-center">
                <h3 class="font-bold">Chưa có sản phẩm nào trong giỏ hàng</h3>
                <div>
                    Cùng mua sắm hàng ngàn sản phẩm tại nhà thuốc {{$fcSystem['homepage_brandname']}} nhé!
                </div>
            </div>
            <div>
                <a href="{{url('/')}}" class="py-[10px] bg-blue-500 hover:bg-blue-700 text-white font-bold flex px-5 rounded-full">Mua hàng</a>
            </div>
        </div>
        @endif
    </div>
