...
$so_dang_ky = !empty(old('so_dang_ky')) ? old('so_dang_ky') : (!empty($detail->so_dang_ky) ? $detail->so_dang_ky : '');
$hoat_chat = !empty(old('hoat_chat')) ? old('hoat_chat') : (!empty($detail->hoat_chat) ? $detail->hoat_chat : '');
$ham_luong = !empty(old('ham_luong')) ? old('ham_luong') : (!empty($detail->ham_luong) ? $detail->ham_luong : '');
$nuoc_san_xuat = !empty(old('nuoc_san_xuat')) ? old('nuoc_san_xuat') : (!empty($detail->nuoc_san_xuat) ? $detail->nuoc_san_xuat : '');
$quy_cach_dong_goi = !empty(old('quy_cach_dong_goi')) ? old('quy_cach_dong_goi') : (!empty($detail->quy_cach_dong_goi) ? $detail->quy_cach_dong_goi : '');
$duong_dung = !empty(old('duong_dung')) ? old('duong_dung') : (!empty($detail->duong_dung) ? $detail->duong_dung : '');
...
<div class=" box p-5">
    ...
    <div class="mt-3">
        <label class="form-label text-base font-semibold">Số đăng ký</label>
        <?php echo Form::text('so_dang_ky', $so_dang_ky, ['class' => 'form-control w-full ']); ?>
    </div>
    <div class="mt-3">
        <label class="form-label text-base font-semibold">Hoạt chất</label>
        <?php echo Form::textarea('hoat_chat', $hoat_chat, ['class' => 'form-control w-full ']); ?>
    </div>
    <div class="mt-3">
        <label class="form-label text-base font-semibold">Hàm lượng</label>
        <?php echo Form::textarea('ham_luong', $ham_luong, ['class' => 'form-control w-full ']); ?>
    </div>
    <div class="mt-3">
        <label class="form-label text-base font-semibold">Nước sản xuất</label>
        <?php echo Form::text('nuoc_san_xuat', $nuoc_san_xuat, ['class' => 'form-control w-full ']); ?>
    </div>
    <div class="mt-3">
        <label class="form-label text-base font-semibold">Quy cách đóng gói</label>
        <?php echo Form::text('quy_cach_dong_goi', $quy_cach_dong_goi, ['class' => 'form-control w-full ']); ?>
    </div>
    <div class="mt-3">
        <label class="form-label text-base font-semibold">Đường dùng</label>
        <?php echo Form::text('duong_dung', $duong_dung, ['class' => 'form-control w-full ']); ?>
    </div>
</div>
<!-- start: Album Ảnh -->
...
<div class=" box p-5 mt-3 space-y-3 <?php if (!in_array('ships', $dropdown)) { ?>hidden<?php } ?>">
    ...
    <!-- END -->
</div><?php /**PATH /home/catdailoc/domains/daicatloc.nhathuocdaicatloc.vn/public_html/resources/views/product/backend/product/common/_detail.blade.php ENDPATH**/ ?>