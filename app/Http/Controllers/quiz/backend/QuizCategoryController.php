...
class QuizCategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $module = $this->table;
        $data =  QuizCategory::where('alanguage', config('app.locale'))->with('user')->with('quizzes')->orderBy('lft', 'ASC');
        ...
        return view('quiz.backend.category.index', compact('data', 'module', 'configIs'));
    }
    ...
    public function create()
    {
        ...
        return view('quiz.backend.category.create', compact('module', 'htmlCatalogue', 'field'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('quiz_categories.index')->with('success', "Thêm mới nhóm đề thi thành công");
    }

    public function edit($id)
    {
        $module = $this->table;
        $detail  = QuizCategory::where('alanguage', config('app.locale'))->find($id);
        if (!isset($detail)) {
            return redirect()->route('quiz_categories.index')->with('error', "Nhóm đề thi không tồn tại");
        }
        $htmlCatalogue = $this->nestedsetbie->dropdown([], config('app.locale'));
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $module])->get();
        return view('quiz.backend.category.edit', compact('detail', 'htmlCatalogue', 'module', 'field'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('quiz_categories.index')->with('success', "Cập nhập nhóm đề thi thành công");
    }
    ...
    public function submit($request = [], $action = '', $id = 0, $arrayImg = [])
    {
        ...
        if ($action == 'create') {
            $id = QuizCategory::insertGetId($_data);
        } else {
            QuizCategory::find($id)->update($_data);
        }
        if (!empty($id)) {
            //xóa khi cập nhập
            if ($action == 'update') {
