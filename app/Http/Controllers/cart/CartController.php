...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        if ($type == 'variable') {
            $getVersionProduct = ProductVersion::select('*', '_stock_status as inventory', '_outstock_status as inventoryPolicy')
                ->where(['product_id' => $id, 'id_version' => $id_version])
                ->with('product_stocks')
                ->first();
        ...
        if (isset($cart[$rowid])) {
            ...
            $cart[$rowid]['price_old'] = $priceProduct['price_old_none_format'];
            ...
            if ($type == 'simple') {
                ...
                $cart[$rowid] = [
                    ...
                    'price_old' => $priceProduct['price_old_none_format'],
                    "ships" => $ships,
                    "unit" => $product->unit,
                ];
            } else {
            ...
            if (isset($coupon)) {
                foreach ($coupon as $v) {
                    ...
                    $html .= '<tr>
                        ...
                        <td>-<span class="amount cart-coupon-price">' . number_format($v['price'], '0', ',', '.') . '₫</span> <a href="javascript:void(0)" data-id="' . $v['id'] . '" class="remove-coupon">[' . trans('index.Delete') . ']</a></td>
                    </tr>';
                }
            }
