...
class ReceiptVouchersController extends Controller
{
    ...
    protected $module = 'receipt_vouchers';
    public function __construct()
    {
        ...
        $categories = dropdown(ReceiptGroups::where('id', '!=', 11)->get(), 'Chọn loại phiếu thu', 'id', 'title');
    ...
    public function index(Request $request)
    {
        ...
        $data = ReceiptVouchers::select($this->module . '.*')->orderBy($this->module . '.id', 'desc')->with(['receipt_groups', 'product_purchases', 'users', 'customers']);
        if (is($module)) {
            $data =  $data->where($this->module . '.module', $module);
        }
        if (is($keyword)) {
            $data = $data->where($this->module . '.code', 'like', '%' . $keyword . '%')
                ->orWhere($this->module . '.reference', 'like', '%' . $keyword . '%')
                ->orWhere($this->module . '.description', 'like', '%' . $keyword . '%')
                ->orWhere('product_purchases.code', 'like', '%' . $keyword . '%')
                ->join('product_purchases', $this->module . '.module_id', '=', 'product_purchases.id');
        }
        if (is($created_at)) {
            ...
            if ($date[0] != $date[1]) {
                $data =  $data->where($this->module . '.created_at', '>=', $date_start)->where($this->module . '.created_at', '<=', $date_end);
            }
        }
        ...
        return view('cashbook.receipt.receipt.index', compact('data', 'users'));
    }
    ...
    public function create()
    {
        ...
        return view('cashbook.receipt.receipt.create', compact('data'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'price.required' => 'Hình thức thanh toán là trường bắt buộc.',
        ]);
        $id = ReceiptVouchers::insertGetId([
            'module' => $request->module,
        ...
        if ($id > 0) {
            return redirect()->route('receipt_vouchers.index')->with('success', "Thêm mới phiếu thu thành công");
        } else {
            return redirect()->route('receipt_vouchers.create')->with('error', "Thêm mới phiếu thu không thành công");
        }
    }
    public function edit($id)
    {
        $detail = ReceiptVouchers::find($id);
        if (!isset($detail)) {
            return redirect()->route('receipt_vouchers.index')->with('error', "phiếu thu không tồn tại");
        }
        $table = $detail->module;
        ...
        return view('cashbook.receipt.receipt.edit', compact('detail', 'data'));
    }
    public function update(Request $request, $id)
    {
        ...
        try {
            ReceiptVouchers::where('id', $id)->update([
                'code' => !empty($request->code) ? $request->code : '',
            ...
            return redirect()->route('receipt_vouchers.index')->with('success', "Cập nhập phiếu thu thành công");
    ...
    public function ajaxSearch(Request $request)
    {
        ...
        $data = ReceiptVouchers::select('receipt_vouchers.*')->orderBy('receipt_vouchers.id', 'desc')->with(['receipt_groups', 'product_purchases', 'users', 'customers']);
        if (is($module)) {
            $data =  $data->whereIn('receipt_vouchers.module', $module);
        }
        if (is($type)) {
            $data =  $data->whereIn('receipt_vouchers.type', $type);
        }
        if (is($group_id)) {
            $data =  $data->whereIn('receipt_vouchers.group_id', $group_id);
        }
        if (is($address_id)) {
            $data =  $data->whereIn('receipt_vouchers.address_id', $address_id);
        }
        if (is($userid_created)) {
            $data =  $data->where('receipt_vouchers.userid_created', $userid_created);
        }
        if (is($status)) {
            $data =  $data->where('receipt_vouchers.status', $status);
        }
        if (!empty($checked)) {
            if ($checked != '-1') {
        ...
        if (is($keyword)) {
            $data = $data->where('receipt_vouchers.code', 'like', '%' . $keyword . '%')
                ->orWhere('receipt_vouchers.reference', 'like', '%' . $keyword . '%')
                ->orWhere('product_purchases.code', 'like', '%' . $keyword . '%')
                ->join('product_purchases', 'receipt_vouchers.module_id', '=', 'product_purchases.id');
        }
        if (is($created_at)) {
            $date =  explode(' to ', $created_at);
            $date_start = trim($date[0] . ' 00:00:00');
            $date_end = trim($date[1] . ' 23:59:59');
            if ($date[0] != $date[1]) {
                $data =  $data->where('receipt_vouchers.created_at', '>=', $date_start)->where('receipt_vouchers.created_at', '<=', $date_end);
            }
        }
        $data = $data->paginate(env('APP_paginate'));
        return view('cashbook.receipt.receipt.data', compact('data'))->render();
    }
    public function ajaxDelete(Request $request)
    {
        ...
        if (empty($id_checked)) {
            ...
            $data = ReceiptVouchers::whereIn('id', $id_checked)->get();
            if (!$data->isEmpty()) {
                foreach ($data as $item) {
                    if ($item->group_id == 11) {
                        return response()->json(array('error' => 'Không được hủy các phiếu thu Tự động, Thanh toán cho đơn nhập hàng và trạng thái “Đã hủy”'));
                    }
                    ...
                    ReceiptVouchers::where('id', $item->id)->update(['status' => 'cancelled', 'updated_at' => Carbon::now()]);
                }
                return response()->json(array('success' => 'Hủy phiếu thu thành công'));
            } else {
                return response()->json(array('error' => 'Bạn phải chọn ít nhất 1 bản ghi để thực hiện chức năng này'));
            }
        }
